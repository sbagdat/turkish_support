...
describe "TurkishSupportHelpers" do
  ...
  describe "#prepare_for" do
    context ":upcase" do
      it "replaces unsupported lower case letters with their upper case's" do
        expect(prepare_for(:upcase, 'çğıiöşü')).to eq('ÇĞIİÖŞÜ')
      end
    end

    context ":downcase" do
      it "replaces unsupported upper case letters with their lower case's" do
        expect(prepare_for(:downcase, 'ÇĞIİÖŞÜ')).to eq('çğıiöşü')
      end
    end
    ...
    context ":capitalize" do
      it "replaces unsupported upper case and lower case letters" do
        dbl = double
        allow(dbl).to receive(:prepare_for).
                        with(:capitalize, 'çaĞLAyan').
                        and_return('Çağlayan')

        expect(dbl.prepare_for(:capitalize, 'çaĞLAyan')).to eq('Çağlayan')
      end
    end

    context "with invalid parameters" do
      it "raises an error if any parameter is not valid" do
        expect{ prepare_for(:not_valid, :not_valid) }.to raise_error ArgumentError
        expect{ prepare_for(:swapcase, 'ÇĞ') }.to raise_error ArgumentError
        expect{ prepare_for(:upcase, 123) }.to raise_error ArgumentError
      end
    end
  end
  ...
  describe "#translate_range" do
    ...
    it "translates ranges created using Turkish characters" do
      ...
      expect(translate_range('ç-ü', true)).to       eq('çdefgğhıijklmnoöpqrsştuüÇĞIİÖŞÜ')
    end
  end
  ...
  describe "#translate_regexp" do
    ...
    it "does not translate ranges outside character class" do
      expect(translate_regexp(/[a-h]+\se-h[ç-ş]afse-h+/)).to eq(/[abcçdefgğh]+\se-h[çdefgğhıijklmnoöpqrsş]afse-h+/)
    end
  end
end
